...
def evaluate_dataset(shared_slm, dataset, evaluator):
    ...
    for i, sample in enumerate(dataset):
        ...
        # Direct prediction without RAG, using exact same template as training
...
def run_federated_simulation_no_rag(num_clients, num_rounds, epochs, split_type, alpha):
    print(f"\nSTARTING SEPARATE FEDERATED EXPERIMENTS (NO RAG): {num_clients} Clients | {num_rounds} Rounds | {epochs} Epochs | {split_type.upper()} | Alpha = {alpha if split_type == 'non-iid' else 'NA'}")
    ...
    file_name = f"eval_results_no_RAG_{num_clients}clients_{num_rounds}rounds_{split_type.upper()}_a{alpha_str}.json"
    ...
    results_dict = {
        "Experiment_Config": {
            ...
            "Model_Type": "Federated Learning (No RAG)"
        },
        "Training_Stats": {},
        "Results": {
            "Fed-SLM (No RAG)": {}
    ...
    checkpoint_vr = f"./model_checkpoints/lora_vr_no_RAG_{num_clients}clients_{num_rounds}rounds_{epochs}epochs_{split_type}_a{alpha_str}.pt"
    ...
    print("\nEvaluating VQA-RAD (No RAG)...")
    vr_c, vr_o, vr_t = evaluate_dataset(shared_slm, vqa_rad_eval, evaluator)
    
    results_dict["Results"]["Fed-SLM (No RAG)"]["VQA-RAD"] = format_scores_for_json(vr_c, vr_o)
    ...
    checkpoint_pv = f"./model_checkpoints/lora_pv_no_RAG_{num_clients}clients_{num_rounds}rounds_{epochs}epochs_{split_type}_a{alpha_str}.pt"
    ...
    # Evaluate PathVQA
    shared_slm.model.load_state_dict(global_weights, strict=False)
    print("\nEvaluating PathVQA (No RAG)...")
    pv_c, pv_o, pv_t = evaluate_dataset(shared_slm, path_vqa_eval, evaluator)
    
    results_dict["Results"]["Fed-SLM (No RAG)"]["PathVQA"] = format_scores_for_json(pv_c, pv_o)
    results_dict["Results"]["Fed-SLM (No RAG)"]["Inference_Time_Seconds"] = round(vr_t + pv_t, 2)
...
if __name__ == "__main__":
    ...
    run_federated_simulation_no_rag(clients_input, rounds_input, epochs_input, split_input, alpha_input)