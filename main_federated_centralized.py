...
def run_centralized_simulation(num_clients, num_rounds, epochs, split_type, alpha):
    ...
    splitter = FederatedDataSplitter(vqa_rad_full, num_clients=num_clients, seed=splitter_seed)
    ...
    rag_subset_size = min(2000, len(vqa_rad_centralized))
    vqa_rad_for_rag = vqa_rad_centralized.shuffle(seed=eval_seed+2).select(range(rag_subset_size))
    path_vqa_for_rag = path_vqa_full.shuffle(seed=eval_seed+3).select(range(min(2000, len(path_vqa_full))))
