...
def run_federated_simulation(num_clients, num_rounds, epochs, split_type, alpha, question_type="all", max_samples=None):
    ...
    file_name = f"eval_results_{num_clients}clients_{num_rounds}rounds_{split_type.upper()}_a{alpha_str}_textonly_SLAKE.json"
    ...
    print(f"\n==================== EXPERIMENT: SLAKE ({num_clients} CLIENTS) ====================")
...
def get_user_setup():
    print("=== Federated Learning Multi-Client Simulation (SLAKE: 2 to 5 Clients) ===")
    
    while True:
        try:
            num_rounds = int(input("1. Enter the max communication rounds (e.g., 5, 10, 20): "))
    ...
    return num_rounds, epochs, split_type, alpha, question_type, max_samples

if __name__ == "__main__":
    rounds_input, epochs_input, split_input, alpha_input, qtype_input, max_samples_input = get_user_setup()
    
    alpha_str = str(alpha_input) if split_input == 'non-iid' else "NA"
    csv_filename = f"summary_results_2to5clients_{split_input.upper()}_a{alpha_str}_SLAKE.csv"
    csv_path = os.path.join("./data", csv_filename)
    
    all_experiment_results = []
    
    for k in range(2, 6): # Automate for 2, 3, 4, 5 clients
        print(f"\n==================================================================")
        print(f"   STARTING SIMULATION FOR K = {k} CLIENTS")
        print(f"==================================================================")
    ...
    print(f"\n==================================================================")
    print(f"   ALL EXPERIMENTS (2 TO 5 CLIENTS) COMPLETED SUCCESSFULLY!")
