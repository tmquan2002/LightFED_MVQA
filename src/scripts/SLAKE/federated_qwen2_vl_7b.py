...
class QwenMedVQA:
    ...
    def predict(self, question, image=None, context=""):
        content = []
    ...
    def train_local(self, shared_slm, epochs=1):
        ...
        for epoch in range(epochs):
            ...
            for i, idx in enumerate(epoch_indices):
                ...
                content = []
                if image is not None:
                    img = shared_slm._preprocess_image(image)
                    if img.mode != "RGB":
                        img = img.convert("RGB")
                    img.thumbnail((336, 336), Image.Resampling.LANCZOS)
                    content.append({"type": "image", "image": img})
...
def evaluate_dataset(shared_slm, dataset, rag_contexts, evaluator, question_type="all"):
    ...
    for i, sample in enumerate(dataset):
        ...
        pred = shared_slm.predict(question, image=image, context=context)
...
def run_federated_simulation(num_clients, num_rounds, epochs, split_type, alpha, question_type="all", max_samples=None):
    print(f"\nSTARTING DECOUPLED FL (QWEN2-VL-7B WITH RAG): {num_clients} Clients | {num_rounds} Rounds | {epochs} Epochs | {split_type.upper()} | Alpha = {alpha if split_type == 'non-iid' else 'NA'} | Question Type = {question_type.upper()} | Max Samples = {max_samples if max_samples is not None else 'ALL'}")
    ...
    torch.cuda.empty_cache()
    gc.collect()
    print("\n[RAG] BiomedCLIP freed from GPU. Proceeding to load Qwen2-VL-7B...")

    os.makedirs("./data", exist_ok=True)
    alpha_str = str(alpha) if split_type == 'non-iid' else "NA"
    file_name = f"eval_results_{num_clients}clients_{num_rounds}rounds_{split_type.upper()}_a{alpha_str}_Qwen2VL7B_SLAKE.json"
    ...
    results_dict = {
        "Experiment_Config": {
            ...
            "Model_Type": "Decoupled FL (Qwen2-VL-7B) with RAG"
        },
        "Training_Stats": {},
        "Results": {
            "Proposed (Fed Qwen2-VL-7B)": {}
        }
    }
    ...
    shared_slm = QwenMedVQA(model_id="Qwen/Qwen2-VL-7B-Instruct", use_4bit=True)
    ...
    print(f"\n==================== EXPERIMENT: SLAKE ({num_clients} CLIENTS - QWEN2-VL-7B) ====================")
    shared_slm.model.load_state_dict(initial_lora_weights, strict=False)
    
    checkpoint_slake = f"./model_checkpoints/lora_slake_{num_clients}clients_{num_rounds}rounds_{epochs}epochs_{split_type}_a{alpha_str}_Qwen2VL7B.pt"
    ...
    results_dict["Results"]["Proposed (Fed Qwen2-VL-7B)"]["SLAKE_Validation"] = format_scores_for_json(val_c, val_o, question_type=question_type)

    print("\nEvaluating SLAKE Test Set (Decoupled Fed Qwen2-VL-7B)...")
    pv_c, pv_o, pv_t = evaluate_dataset(shared_slm, vqa_rad_eval, eval_rag_contexts, evaluator, question_type=question_type)
    
    results_dict["Results"]["Proposed (Fed Qwen2-VL-7B)"]["SLAKE_Test"] = format_scores_for_json(pv_c, pv_o, question_type=question_type)
    results_dict["Results"]["Proposed (Fed Qwen2-VL-7B)"]["Inference_Time_Seconds"] = round(pv_t, 2)
...
def save_summary_csv(results_list, csv_path):
    ...
    for res in results_list:
        ...
        fed_res = res.get("Results", {}).get("Proposed (Fed Qwen2-VL-7B)", {})
...
def get_user_setup():
    print("=== Federated Learning Multi-Client Simulation (SLAKE - Qwen2-VL-7B) ===")
...
if __name__ == "__main__":
    ...
    csv_filename = f"summary_results_{num_clients_input}clients_{split_input.upper()}_a{alpha_str}_Qwen2VL7B_SLAKE.csv"
    csv_path = os.path.join("./data", csv_filename)
    
    print(f"\n==================================================================")
    print(f"   STARTING SIMULATION FOR {num_clients_input} CLIENTS (QWEN2-VL-7B)")
    print(f"==================================================================")
    ...
    print(f"\n==================================================================")
    print(f"   SIMULATION ({num_clients_input} CLIENTS - QWEN2-VL-7B) COMPLETED SUCCESSFULLY!")
