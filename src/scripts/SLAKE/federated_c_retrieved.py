...
def run_federated_simulation(num_clients, num_rounds, epochs, split_type, alpha, question_type="all", max_samples=None, c_retrieved=3):
    print(f"\nSTARTING DECOUPLED FL (TEXT-ONLY WITH RAG): {num_clients} Clients | C = {c_retrieved} Retrieved Docs | {num_rounds} Rounds | {epochs} Epochs | {split_type.upper()} | Alpha = {alpha if split_type == 'non-iid' else 'NA'} | Question Type = {question_type.upper()} | Max Samples = {max_samples if max_samples is not None else 'ALL'}")
    ...
    # Precompute RAG Contexts (BiomedCLIP) with top-C retrieved documents
    print(f"\n[RAG] Loading BiomedCLIP to precompute FAISS indices and top-{c_retrieved} contexts...")
    ...
    # Global Eval RAG
    print(f"  Precomputing for Global Evaluation (c={c_retrieved})...")
    ...
    for i in range(len(vqa_rad_eval)):
        cases = retriever_global.search_cases(eval_queries[i], c=c_retrieved)
        ctx = "\n".join([f"- Question: {c['question']}\n  Answer: {c['answer']}" for c in cases])
        eval_rag_contexts.append(ctx)

    print(f"  Precomputing for Validation (c={c_retrieved})...")
    ...
    for i in range(len(vqa_rad_val)):
        cases = retriever_global.search_cases(val_queries[i], c=c_retrieved)
        ctx = "\n".join([f"- Question: {c['question']}\n  Answer: {c['answer']}" for c in cases])
    ...
    for i, ds in enumerate(client_datasets_rad):
        print(f"  Precomputing for Hospital {i+1} (c={c_retrieved})...")
        ...
        for j in range(len(ds)):
            cases = retriever_local.search_cases(local_embeds[j], c=c_retrieved, avoid_self_idx=j)
            ctx = "\n".join([f"- Question: {c['question']}\n  Answer: {c['answer']}" for c in cases])
            rag_ctx.append(ctx)
    ...
    file_name = f"eval_results_{num_clients}clients_c{c_retrieved}_{num_rounds}rounds_{split_type.upper()}_a{alpha_str}_textonly_SLAKE.json"
    json_path = os.path.join("./data", file_name)
    
    results_dict = {
        "Experiment_Config": {
            "Num_Clients": num_clients,
            "C_Retrieved_Docs": c_retrieved,
            "Max_Rounds_Configured": num_rounds,
            "Local_Epochs": epochs,
            "Split_Type": split_type.upper(),
            "Alpha": alpha if split_type == 'non-iid' else "NA",
            "Model_Type": f"Decoupled FL (Text-only Qwen) with RAG (c={c_retrieved})"
        },
        "Training_Stats": {},
        "Results": {
            "Proposed (Fed+RAG)": {}
    ...
    print(f"\n==================== EXPERIMENT: SLAKE ({num_clients} CLIENTS | C = {c_retrieved}) ====================")
    shared_slm.model.load_state_dict(initial_lora_weights, strict=False)
    
    checkpoint_slake = f"./model_checkpoints/lora_slake_{num_clients}clients_c{c_retrieved}_{num_rounds}rounds_{epochs}epochs_{split_type}_a{alpha_str}_textonly.pt"
    ...
    results_dict["Results"]["Proposed (Fed+RAG)"]["SLAKE_Validation"] = format_scores_for_json(val_c, val_o, question_type=question_type)

    print("\nEvaluating SLAKE Test Set (Decoupled Fed+RAG)...")
    pv_c, pv_o, pv_t = evaluate_dataset(shared_slm, vqa_rad_eval, eval_rag_contexts, evaluator, question_type=question_type)
    
    results_dict["Results"]["Proposed (Fed+RAG)"]["SLAKE_Test"] = format_scores_for_json(pv_c, pv_o, question_type=question_type)
...
def save_summary_csv(results_list, csv_path):
    ...
    for res in results_list:
        ...
        row = {
            ...
            "C_Retrieved_Docs": cfg.get("C_Retrieved_Docs", ""),
...
def get_user_setup():
    print("=== Federated Learning Multi-Client Simulation (SLAKE: Loop C = 2, 3, 5, 10 Retrieved Documents) ===")
...
if __name__ == "__main__":
    ...
    c_list = [2, 3, 5, 10]
    csv_filename = f"summary_results_{num_clients_input}clients_c_2_3_5_10_{split_input.upper()}_a{alpha_str}_SLAKE.csv"
    ...
    for c in c_list:
        print(f"\n==================================================================")
        print(f"   STARTING SIMULATION FOR C = {c} RETRIEVED DOCUMENTS (NUM_CLIENTS = {num_clients_input})")
        print(f"==================================================================")
        ...
        # Save CSV summary after each iteration so progress is saved incrementally
        save_summary_csv(all_experiment_results, csv_path)
        
        clear_memory()
        
    print(f"\n==================================================================")
    print(f"   ALL EXPERIMENTS (C = 2, 3, 5, 10 RETRIEVED DOCUMENTS) COMPLETED SUCCESSFULLY!")
