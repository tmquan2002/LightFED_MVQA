...
def run_centralized_training(epochs, question_type="all", max_samples=None):
    ...
    if question_type in ["closed", "open"]:
        indices = []
        for idx, item in enumerate(path_vqa_train):
            is_closed = is_closed_ended(item['answer'])
            if question_type == "closed" and is_closed:
                indices.append(idx)
            elif question_type == "open" and not is_closed:
                indices.append(idx)
        path_vqa_train = path_vqa_train.select(indices)
        print(f"[Data] Filtered dataset to {question_type} questions: {len(path_vqa_train)} samples.")
        
    if max_samples is not None and len(path_vqa_train) > max_samples:
        random.seed(42)
        indices = random.sample(range(len(path_vqa_train)), max_samples)
        indices.sort()
        path_vqa_train = path_vqa_train.select(indices)
        print(f"[Data] Limited dataset to {max_samples} samples.")
        
    print(f"Training on {len(path_vqa_train)} samples from PathVQA.")
    ...
    file_name = f"centralized_results_epochs_{epochs}.json"
    ...
    print(f"\n==================== EXPERIMENT: PathVQA Centralized Training ====================")
    
    import torch.optim as optim
    from qwen_vl_utils import process_vision_info
    
    optimizer = optim.AdamW(filter(lambda p: p.requires_grad, slm.model.parameters()), lr=2e-5)
    ...
    rag_cache = {}
    for idx in range(len(path_vqa_train)):
        sample = path_vqa_train[idx]
        retrieved = retriever_pv.search_similar_cases(sample['image'], query_question=sample['question'], c=4)
        rag_cache[idx] = [case for case in retrieved if case['id'] != idx][:3]

    indices = list(range(len(path_vqa_train)))
    ...
    for epoch in range(epochs):
        ...
        for i, idx in enumerate(indices):
            ...
            for j, case in enumerate(similar_cases):
                try:
                    ref_img = slm._preprocess_image(case['image'])
                    ref_img = optimize_img(ref_img)
                    messages.append({
                        "role": "user",
                        "content": [
            ...
            target_img = slm._preprocess_image(sample['image'])
            target_img = optimize_img(target_img)
            messages.append({
                "role": "user",
                "content": [
            ...
            text = slm.processor.apply_chat_template(messages, tokenize=False, add_generation_prompt=False)
            image_inputs, video_inputs = process_vision_info(messages)
            ...
            prompt_text = slm.processor.apply_chat_template(messages[:-1], tokenize=False, add_generation_prompt=True)
            prompt_inputs = slm.processor(
                text=[prompt_text],
                images=image_inputs,
                videos=video_inputs,
                padding=True,
            ...
            if i % 10 == 0:
                print(f"  Epoch {epoch+1}/{epochs} | Step {i}/{len(indices)} | Loss: {loss.item() * 16:.4f}   ", end="\r")
                
        print(f"  Epoch {epoch+1}/{epochs} Completed | Avg Loss: {total_loss / steps if steps > 0 else 0.0:.4f}")
        
    total_train_time = round(time.time() - start_train_time, 2)
    results_dict["Training_Stats"]["PathVQA"] = {
        "Final_Average_Loss": round(total_loss / steps if steps > 0 else 0.0, 4),
        "Training_Time_Seconds": total_train_time
    }
    
    print("\nEvaluating PathVQA (Centralized+RAG)...")
    pv_c, pv_o, pv_t = evaluate_dataset(slm, path_vqa_eval, evaluator, retriever_pv, question_type=question_type)
    
    results_dict["Results"]["Centralized+RAG"]["PathVQA"] = format_scores_for_json(pv_c, pv_o, question_type=question_type)
    results_dict["Results"]["Centralized+RAG"]["Inference_Time_Seconds"] = round(pv_t, 2)
...
def get_user_setup():
    while True:
        try:
            epochs = int(input("1. Enter training Epochs (e.g., 1, 2, 3 - 1 is recommended for speed/stability): "))
            if epochs >= 1: break
